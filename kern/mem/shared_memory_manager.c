...
int createSharedObject(int32 ownerID, char* shareName, uint32 size, uint8 isWritable, void* virtual_address)
{
	...
	struct Share *ptr1 = NULL;
	int ShareID = allocate_share_object(&ptr1);
	if (ShareID != E_NO_SHARE)
	{
		...
		for (int i = 0; i < NoOfPAGES; i++)
		{
			...
			if (ret != E_NO_MEM)
			{
				...
				map_frame(myenv->env_page_directory, frame,(uint32)virtual_address + (i * PAGE_SIZE), perm);
				add_frame_to_storage(ptr1->framesStorage, frame, i);
			}
		}
		ptr1->ownerID = ownerID;
		ptr1->isWritable = isWritable;
		strcpy(ptr1->name, shareName);
		ptr1->size = size;
		ptr1->references = 1;
		return ShareID;
	}
...
int getSharedObject(int32 ownerID, char* shareName, void* virtual_address)
{
    ...
    int Object = get_share_object_ID(ownerID, shareName);
	if (Object != E_SHARED_MEM_NOT_EXISTS)
	{
	int back=0;
	struct Share *shareObj = &shares[Object];
	int x = ROUNDUP(shareObj->size,PAGE_SIZE) / PAGE_SIZE;
	for (int i = 0; i < x; i++) {
		struct FrameInfo* ptr_frame = get_frame_from_storage(shareObj->framesStorage, back);
		int perm = PERM_PRESENT | PERM_USER;
		if (shareObj->isWritable == 1)
		{
			perm |= PERM_WRITEABLE;
		}
		map_frame(myenv->env_page_directory, ptr_frame,(uint32)virtual_address + (i * PAGE_SIZE), perm);
		back++;
	}
	shareObj->references++;
	return Object;
...
int freeSharedObject(int32 sharedObjectID, void *startVA)
{
	...
	//uint32 start = (uint32) startVA;
     //int ret=free_share_object(sharedObjectID);
