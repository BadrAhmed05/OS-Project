...
int createSemaphore(int32 ownerEnvID, char* semaphoreName, uint32 initialValue)
{
	...
	int ptr = get_semaphore_object_ID(ownerEnvID,semaphoreName);
    if(ptr == E_SEMAPHORE_NOT_EXISTS){
    	struct Semaphore* sptr = NULL;
    	ptr = allocate_semaphore_object(&sptr);
    	if(ptr == E_NO_SEMAPHORE){
    			return ptr;
    	}
    			else if(ptr != E_NO_SEMAPHORE){
    				semaphores[ptr].ownerID = ownerEnvID ;
    			    strcpy(semaphores[ptr].name, semaphoreName);
    			    semaphores[ptr].value   = initialValue ;
    				return ptr ;
    			}
    }else
...
void waitSemaphore(int32 ownerEnvID, char* semaphoreName)
{
	...
	int ptr = get_semaphore_object_ID(ownerEnvID,semaphoreName);
	if(ptr == E_SEMAPHORE_EXISTS){
		semaphores[ptr].value--;
		if(semaphores[ptr].value < 0){
			enqueue(&semaphores[ptr].env_queue,myenv);
...
void signalSemaphore(int ownerEnvID, char* semaphoreName)
{
	...
	int ptr = get_semaphore_object_ID(ownerEnvID,semaphoreName);
		if(ptr == E_SEMAPHORE_EXISTS){
			semaphores[ptr].value++;
			if(semaphores[ptr].value <= 0){
				myenv =  dequeue(&semaphores[ptr].env_queue);
